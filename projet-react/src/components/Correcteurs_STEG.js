...
function Correcteurs_STEG() {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/api/components/Correcteurs_STEG')
      .then(response => {
  ...
  return (
    <div className="container">
      <div className="mb-4">
        <Link to="/AddCorrecteurSTEG" className="btn btn-success">Ajouter un nouveau Correcteur_STEG</Link>
      </div>
            ...
            <tbody>
              {filteredData.map(item => (
                <tr key={item['Désignation de l\'appareil/N° Série']}>
                  ...
                  <td>{item['Etat']}</td>
                  <td>
                    <Link to={`/details_Correcteurs_STEG/${item['Désignation de l\'appareil/N° Série']}`} className="btn btn-primary">Voir détails</Link>
                  </td>
                </tr>
              ))}
            </tbody>
...
export default Correcteurs_STEG;