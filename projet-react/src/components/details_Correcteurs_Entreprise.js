...
function Details_Correcteurs_Entreprise() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/components/Correcteurs_Entreprise/${id}`);
  ...
  const handleDelete = async () => { 
    try {
      await axios.delete(`http://localhost:5000/api/components/Correcteurs_Entreprise/${id}`);
      alert('Ressource supprimée avec succès');
      navigate('/components/Correcteurs_Entreprise'); // Utilisation de navigate pour rediriger après la suppression
    } catch (error) {
      ...
      {details ? (
        <div className="row">
          <div className="col-md-6">
            ...
            <p>Désignation de l'appareil :(T / O): {details['Désignation de l\'appareil :T / O']}</p>
...
export default Details_Correcteurs_Entreprise;