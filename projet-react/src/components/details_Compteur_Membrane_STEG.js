...
function Details_Compteur_Membrane_STEG() {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/components/Compteur_Membrane_STEG/${id}`);
  ...
  const handleDelete = async () => { 
    try {
      await axios.delete(`http://localhost:5000/api/components/Compteur_Membrane_STEG/${id}`);
      alert('Ressource supprimée avec succès');
      navigate('/components/Compteur_Membrane_STEG'); // Utilisation de navigate pour rediriger après la suppression
    } catch (error) {
...
export default Details_Compteur_Membrane_STEG;