...
function Details_Compteur_Turbine_STEG() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/components/Compteur_Turbine_Entreprise/${id}`);
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`http://localhost:5000/api/components/Compteur_Turbine_Entreprise/${id}`);
      alert('Ressource supprimée avec succès');
      navigate('/components/Compteur_Turbine_Entreprise'); // Utilisation de navigate pour rediriger après la suppression
    } catch (error) {
