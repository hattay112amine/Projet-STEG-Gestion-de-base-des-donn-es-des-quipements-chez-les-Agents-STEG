...
function Details_Compteur_Piston_Entreprise() {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/components/Compteur_Piston_Entreprise/${id}`);
  ...
  const handleDelete = async () => { 
    try {
      await axios.delete(`http://localhost:5000/api/components/Compteur_Piston_Entreprise/${id}`);
      alert('Ressource supprimée avec succès');
      navigate('/components/Compteur_Piston_Entreprise'); // Utilisation de navigate pour rediriger après la suppression
    } catch (error) {
...
export default Details_Compteur_Piston_Entreprise;