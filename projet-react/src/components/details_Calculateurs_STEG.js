...
function Details_Calculateurs_STEG() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/components/Calculateurs_STEG/${id}`);
  ...
  const handleDelete = async () => { 
    try {
      await axios.delete(`http://localhost:5000/api/components/Calculateurs_STEG/${id}`);
      alert('Ressource supprimée avec succès');
      navigate('/components/Calculateurs_STEG'); // Utilisation de navigate pour rediriger après la suppression
    } catch (error) {
...
export default Details_Calculateurs_STEG;