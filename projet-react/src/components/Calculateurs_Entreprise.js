...
function Calculateurs_Entreprise() {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/api/components/Calculateurs_Entreprise')
      .then(response => {
  ...
  return (
    <div className="container">
      <div className="mb-4">
        <Link to="/AddCalculateurEntreprise" className="btn btn-success">Ajouter un nouveau Calculateur_Entreprise</Link>
      </div>
            ...
            <tbody>
              {filteredData.map(item => (
                <tr key={item['Désignation de l\'appareil/N° Série']}>
                  ...
                  <td>{item['Etat']}</td>
                  <td>
                    <Link to={`/details_Calculateurs_Entreprise/${item['Désignation de l\'appareil/N° Série']}`} className="btn btn-primary">Voir détails</Link>
                  </td>
                </tr>
              ))}
            </tbody>
...
export default Calculateurs_Entreprise;