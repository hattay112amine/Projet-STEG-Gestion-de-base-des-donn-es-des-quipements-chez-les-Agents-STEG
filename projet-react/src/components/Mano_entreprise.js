...
function Mano_entreprise() {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/api/components/Mano_entreprise')
      .then(response => {
  ...
  return (
    <div className="container">
      <div className="mb-4">
        <Link to="/AddManometreEntreprise" className="btn btn-success">Ajouter un nouveau Manometre_Entreprise</Link>
      </div>
            ...
            <tbody>
              {filteredData.map(item => (
                <tr key={item['Désignation de l\'appareil/N° Série']}>
                  ...
                  <td>{item['Etat']}</td>
                  <td>
                    <Link to={`/details/${item['Désignation de l\'appareil/N° Série']}`} className="btn btn-primary">Voir détails</Link>
                  </td>
                </tr>
              ))}
            </tbody>
...
export default Mano_entreprise;