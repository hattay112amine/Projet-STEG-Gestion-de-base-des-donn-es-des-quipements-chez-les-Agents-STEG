...
function Details_Correcteurs_STEG() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/components/Correcteurs_STEG/${id}`);
...
const handleDelete = async () => { 
  try {
    await axios.delete(`http://localhost:5000/api/components/Correcteurs_STEG/${id}`);
    alert('Ressource supprimée avec succès');
    navigate('/components/Correcteurs_STEG'); // Utilisation de navigate pour rediriger après la suppression
  } catch (error) {
...
export default Details_Correcteurs_STEG;